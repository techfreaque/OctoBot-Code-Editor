...
export declare type FnValidatedArgument<T extends [] | [AnyStrictValidator, ...AnyStrictValidator[]]> = T extends [AnyStrictValidator, ...AnyStrictValidator[]] ? {
...
}) => StrictValidator<unknown, ObjectType<{ [P_1 in keyof T]: InferType<T[P_1]>; } & DeriveIndexUnlessNull<UnknownValidator>>>;
export declare const isPartial: <T extends { [P in keyof T]: StrictValidator<any, any>; }>(props: T) => StrictValidator<unknown, ObjectType<{ [P_1 in keyof T]: InferType<T[P_1]>; } & {
    [k: string]: unknown;
}>>;
